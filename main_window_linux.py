...
import sys
import threading
import subprocess # needed for xdotool

from dearpygui import dearpygui as dpg

import formation_extrapolator_new
import formation_list_new
import hook_linux as hook # important: use new Linux-Hook-File
import settings
...
class ConnectEmulatorDialog:
    def on_emulator_select(self):
        ...
        # PIDs have to be converted to string
        dpg.configure_item(self.input_emu_pid, items=[str(p) for p in pids], default_value=str(pids[0])) 
    ...
    def click_connect(self):
        selected_emu = dpg.get_value(self.input_emu_name)
        selected_pid_str = dpg.get_value(self.input_emu_pid)
        ...
        for platform_version in hook.Hook.EMULATOR_MAP[selected_emu][1]:
            if platform_version.name == selected_version:
                ...
                self.parent_app.hook.hooked_process_id = int(selected_pid_str)
                self.parent_app.hook.start()
                dpg.delete_item(self.modal_id)

    def click_show_window(self):
        """Ersetzt win32gui.BringWindowToTop durch xdotool."""
        pid = dpg.get_value(self.input_emu_pid)
        try:
            # xdotool looks for window and gets it to the foreground
            subprocess.run(["xdotool", "search", "--onlyvisible", "--pid", str(pid), "windowactivate", "--sync", "%1"], check=True)
        except FileNotFoundError:
            show_error("Missing dependency", "'xdotool' is needed, but wasn't found.")
        except subprocess.CalledProcessError:
            show_error("Window not found", f"Couldn't find or activate a visible window for PID {pid}")
        except Exception as e:
            show_error("Error", f"unexpected error: {e}")
...
if __name__ == '__main__':
    print("entered main")
    print("if this doesn't work, please try running the script with sudo")
